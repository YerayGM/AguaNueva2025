...
export const getExpedienteById = async (expedienteId: string): Promise<Expediente> => {
  const response = await apiService.get<{success: boolean, data: Expediente}>(`${BASE_URL}/id/${expedienteId}`);
  return response.data;
};

export const getExpedientesByDni = async (dni: string): Promise<Expediente[]> => {
  const response = await apiService.get<{success: boolean, data: Expediente[]}>(`${BASE_URL}/buscar/dni/${dni}`);
  return response.data;
};

export const getExpedientesByMunicipio = async (idMunicipio: number): Promise<Expediente[]> => {
  const response = await apiService.get<{success: boolean, data: Expediente[]}>(`${BASE_URL}/buscar/municipio/${idMunicipio}`);
  return response.data;
};
